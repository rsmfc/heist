/**
 * High-Tech 3D Slingshot Cannon & Aim Trajectory Engine for Vault Heist
 * Features a large, brightly lit 3D Slingshot Cannon with elastic bands, pull-tension mechanics, and glowing aim arc guidance.
 */
...
export class LauncherManager {
  ...
  public aimState: AimState = { pitch: 0.35, yaw: 0.0, power: 0.65 };
  ...
  private leftProngPos = new THREE.Vector3(0, 1.8, -0.8);
  private rightProngPos = new THREE.Vector3(0, 1.8, 0.8);
  ...
  constructor(scene: THREE.Scene, physicsWorld: PhysicsWorld, materials: VaultMaterials) {
    ...
    // 2. Slingshot Pouch Pocket
    const pouchGeom = new THREE.BoxGeometry(0.35, 0.4, 0.5);
  ...
  public getInitialVelocity(): THREE.Vector3 {
    const speed = 18 + Math.pow(this.aimState.power, 1.2) * 34; // Speed scaling from 18 to 52 m/s
